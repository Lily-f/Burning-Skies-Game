...
public class Boss implements Enemy{
  ...
  int points = 10;  //number of points the boss is worth
  float dropPowerupChance = 0.8;
  int health = 10;
  ...
  public Boss(PVector playerPos, PVector spawnPosition){
    
    position.set(spawnPosition.x, spawnPosition.y);
    playerPosition.set(playerPos);
    
    //set the angle and magnitude of the boss so it points at player and moves 'speed' pixels per frame
    velocity.set(playerPosition.x - position.x, playerPosition.y - position.y);
    velocity.setMag(speed);
  }
  
  
  //move the boss based on where the player is
  ...
  //draw the boss based on where the player & boss are on the background
  public void redraw(){
    
    //only draw if the enemy is on the window (help stop lag)
